...
def _pipe_with_edge_cfg(**overrides):
    ...
    return ROSEPipeline(config=cfg)
...
class TestBearing:
    ...
    def test_quantize_8_way(self, angle_deg, expected):
        result = ROSEPipeline._quantize_8_way(math.radians(angle_deg))
        assert result == expected

    def test_boundary_225(self):
        """22.5° is at the boundary: >= 22.5 → front-left."""
        assert ROSEPipeline._quantize_8_way(math.radians(22.5)) == "front-left"

    def test_boundary_negative_225(self):
        """-22.5° is at the boundary: >= -22.5 → front."""
        assert ROSEPipeline._quantize_8_way(math.radians(-22.5)) == "front"

    def test_boundary_1575(self):
        """157.5° → back."""
        assert ROSEPipeline._quantize_8_way(math.radians(157.5)) == "back"

    def test_boundary_negative_1575(self):
        """-157.5° → back-right (>= -157.5, < -112.5)."""
        assert ROSEPipeline._quantize_8_way(math.radians(-157.5)) == "back-right"

    def test_zero_is_front(self):
        assert ROSEPipeline._quantize_8_way(0.0) == "front"

    def test_pi_is_back(self):
        assert ROSEPipeline._quantize_8_way(math.pi) == "back"
