"""Tests for ROSEConfig and YAML loading/saving."""
...
class TestDefaultValues:
    """Verify defaults match docs/roadmap/ROSE_IMPLEMENTATION.md Section 6."""
...
class TestYAMLSaveLoad:
    ...
    def test_save_and_load(self, tmp_path):
        ...
        save_rose_config(original, path)
        loaded = load_rose_config(path)
    ...
    def test_load_nonexistent_raises(self):
        with pytest.raises(FileNotFoundError):
            load_rose_config("/nonexistent/path/cfg.yaml")

    def test_load_empty_yaml(self, tmp_path):
        path = tmp_path / "empty.yaml"
        path.write_text("")
        cfg = load_rose_config(path)
    ...
    def test_save_creates_parent_dirs(self, tmp_path):
        ...
        save_rose_config(ROSEConfig(), path)
        assert path.exists()
...
class TestBuildFromDict:
    ...
    def test_none_input_returns_default(self):
        cfg = _build_from_dict(ROSEConfig, None)
        assert isinstance(cfg, ROSEConfig)
        assert cfg.seed == 42

    def test_empty_dict_returns_default(self):
        cfg = _build_from_dict(ROSEConfig, {})
        assert cfg.seed == 42
...
class TestMutability:
    ...
    def test_config_fields_mutable(self):
        """ROSEConfig is NOT frozen — fields can be changed at runtime."""
        c = ROSEConfig()
        c.seed = 999
        assert c.seed == 999
        c.step.grid_size = 32
