#!/usr/bin/env python3
"""Smoke test for ROSE pipeline with mock vision outputs (no GPU needed).
...
def test_result_cleanup():
    ...
    Tests both ROSEE2EResult and ROSE4DSGResult via shared
    ...
    from rose.engine.pipeline import ROSE4DSGResult as Imported
    assert Imported is ROSE4DSGResult, "ROSE4DSGResult must be importable from package"

    print("  ROSE4DSGResult importable from rose.engine.pipeline")
    print("  PASSED")
...
def test_max_tau_per_step_truncation():
    ...
    config_unlimited = ROSEConfig()
    config_unlimited.step.max_tau_per_step = 0  # unlimited
    pipeline_unlimited = ROSEPipeline(config_unlimited)
