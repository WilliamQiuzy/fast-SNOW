...
DA3_MODEL = ROOT / "rose" / "models" / "da3-small"
SAM3_MODEL = ROOT / "rose" / "models" / "sam3"
...
def benchmark_yolo(frames: list):
    ...
    import torch
    from rose.vision.perception.yolo_wrapper import YoloBBoxDetector
...
def benchmark_cpu_pipeline(frames: list):
    ...
    from rose.engine.config.rose_config import ROSEConfig, DA3Config
    from rose.engine.pipeline.rose_pipeline import (
        ROSEPipeline, FastFrameInput, FastLocalDetection,
    )
    from rose.vision.perception.da3_wrapper import DA3Wrapper
    ...
    from rose.reasoning.tokens.patch_tokenizer import mask_to_patch_tokens
